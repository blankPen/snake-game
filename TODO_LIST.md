# TODO_LIST.md - Snake Game 项目待办列表

> 最后更新: 2026-03-10 20:11

## 项目状态: ✅ v1.1.0 已发布

所有 MVP 功能已完成并发布。

## 已完成功能

| 功能 | 状态 | 关联 Issue |
|------|------|------------|
| 游戏核心逻辑 | ✅ 完成 | #1, #3 |
| UI 界面 | ✅ 完成 | #2 |
| 键盘控制 | ✅ 完成 | #12 |
| 触摸控制 | ✅ 完成 | #13 |
| 暂停/继续 | ✅ 完成 | #14 |
| 难度选择 | ✅ 完成 | #15 |
| 分数统计 | ✅ 完成 | #16 |
| 最高分记录 | ✅ 完成 | #16 |
| 游戏结束界面 | ✅ 完成 | #17 |
| 响应式布局 | ✅ 完成 | #18 |
| Bug 修复 | ✅ 完成 | #19 |
| 成就系统 | ✅ 完成 | #24 |
| 移动端体验优化 | ✅ 完成 | #25 |
| 游戏状态持久化 | ✅ 完成 | #29 |
| 视觉效果增强 | 🔄 开发中 | #31 |
| 项目文档 | 🔄 开发中 | #32 |

## 方向建议（彭震，2026-03-10）

彭震在群里提出了以下功能改进提案：

| 优先级 | Issue | 功能 | 状态 |
|--------|-------|------|------|
| P0 | #37 | 音效系统 | 🔄 开发中 (Alpha) |
| P1 | #38 | 速度渐变系统 | 🔄 开发中 (Beta) |
| P2 | #39 | 道具系统 | 🔄 开发中 (Alpha) |
| P3 | #40 | 动画效果 | 🔄 开发中 (Beta) |

---

## 待开发功能（22 个 Open Issues）

### 高优先级 (P0-P1)

| Issue | 功能 | 标签 | 状态 |
|-------|------|------|------|
| #50 | 事件总线架构重构 | refactor | 🔄 Arch 设计方案中 |
| #57 | 重构 Game.js 将 Renderer 分离 | refactor | 🔄 开发中 (Alpha) |
| #58 | 移动端触摸控制优化 | enhancement | 🔄 开发中 (Beta) |
| #41 | 渲染性能优化 | enhancement | 待开发 (Alpha) |
| #51 | 游戏统计功能 | enhancement | 待开发 (Alpha) |
| #55 | 每日挑战模式 | enhancement | 待开发 |
| #44 | ESLint 代码检查与 CI 自动化 | enhancement | 待开发 (Ops) |

### 中优先级 (P2)

| Issue | 功能 | 标签 | 状态 |
|-------|------|------|------|
| #47 | AI 对手模式 | enhancement | 待开发 (Alpha) |
| #46 | 游戏回放系统 | enhancement | 待开发 (Beta) |
| #36 | 道具系统与关卡模式 | enhancement | 待开发 (Beta) |
| #31 | 视觉效果增强 | enhancement | 🔄 开发中 (Alpha) |
| #32 | 完善项目文档 | documentation | 🔄 开发中 (Alpha) |
| #28 | 单元测试覆盖 | enhancement | 待开发 (Beta) |
| #30 | GitHub Actions CI/CD | enhancement | 待开发 (Ops) |
| #29 | 游戏状态持久化 | enhancement | ✅ 已完成 |

### 低优先级 (P3+)

| Issue | 功能 | 标签 | 状态 |
|-------|------|------|------|
| #42 | 游戏统计功能 | enhancement | 待开发 |
| #49 | 视觉反馈与特效系统 | enhancement | 待开发 |
| #48 | 游戏过程数据记录系统 | enhancement | 待开发 |
| #43 | 修复 gameLoop 时间计算 | bug | ✅ 已修复 |
| #33 | InputHandler 回调不匹配 | bug | ✅ 已修复 |

---

## 团队任务分配

| 成员 | 活跃任务数 | 当前工作 |
|------|-----------|----------|
| Alpha | 5 | 音效系统、道具系统、视觉效果、文档 |
| Beta | 5 | 速度渐变、动画效果、移动端优化、回放系统 |
| Arch | 1 | 事件总线架构设计 |
| QA | 0 | - |
| Ops | 2 | ESLint CI、GitHub Actions |
